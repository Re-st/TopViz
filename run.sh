...
cmd="python3 $BASE/scripts/run_replay.py -i $BASE/safe/dafl-Topuzz-avg/$BIN-float/ --topuzz --test -S >log 2>&1"
echo $cmd
eval $cmd
...
echo "outdir: $OUTDIR"
cmd="python3 $BASE/scripts/parse_replay.py -i $BASE/safe/dafl-Topuzz-avg/$BIN-float -r $OUTDIR/$BIN-topuzz -p 24 -S"
echo $cmd
eval $cmd
